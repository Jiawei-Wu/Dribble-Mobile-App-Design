...
#define PORT "23575"  // the port users will be connecting to
#define PORTa "21575"  // the port users will be connecting to
...
{
    char buf[MAXLINE];
    int sockfd, new_fd, sockrec, n;  // listen on sock_fd, new connection on new_fd
...
int sockfda;
    ...
    if ((rva = getaddrinfo(HOST, PORTa, &hintsa, &servinfoa)) != 0) {
        fprintf(stderr, "getaddrinfo: %s\n", gai_strerror(rva));
        return 1;
    }
    printf("11");
