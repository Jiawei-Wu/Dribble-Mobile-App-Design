...
void deal(char * buf){
	...
	if(operator==1){
		...
		sendto(sockfd2, sendline, strlen(sendline), 0, (struct sockaddr *)&servaddr, sizeof(servaddr));
		recvfrom(sockfd, recvline, LEN+1, 0, NULL, NULL);
	}
	strcpy(buf,number1);
	...
	if(operator==1){
		...
		sendto(sockfd2, buf, strlen(buf), 0, (struct sockaddr *)&servaddr, sizeof(servaddr));
	}
